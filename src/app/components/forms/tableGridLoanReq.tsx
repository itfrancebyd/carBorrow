...
import AllocateCarModal from "./allocateForm";
...
const TableCell = ({
    ...
    if (key === "status") {
        return item.status === "new" ? (
            <div className="bg-[#26361C] text-white text-[10px] font-semibold px-1 py-0.5 rounded-sm">NEW</div>
        ...
        <div className="w-full h-full overflow-hidden flex px-6 text-[#494949] text-xs lg:text-sm">
            ...
            {isAllocateOpen &&
                <AllocateCarModal
                    ...
                    onConfirm={function (selectedVehicleId: string): void {
