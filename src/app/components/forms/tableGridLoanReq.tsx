...
const TableCell = ({
    ...
    if (key === "status") {
        if (item.status === "new") {
            return (
                <div className="bg-[#4C763B] text-white text-[10px] font-semibold px-1 py-0.5 rounded-sm">NEW</div>
        ...
        if (item.status === "allocated") {
            return (
                <div className="bg-[#B0CE88] text-white text-[10px] font-semibold px-1 py-0.5 rounded-sm">ALLOCATED</div>
            )
        }
    }
    if (key === "allocate") {
